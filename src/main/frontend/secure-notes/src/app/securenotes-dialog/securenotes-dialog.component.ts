...
const CHOICES: string[] = ["> CIPHER", "> CLEAR"];
...
export class SecurenotesDialog implements OnInit {
  ...
  choose(choice: string) {
    console.log("SecurenotesDialog#choose " + choice);
    this.choosen = true;
    if (choice.trim() === "NEW") {
      this.prepareNew();
    } else {
      this.setButtonStates(true, false, false, false, this.showDatabase, false);
      if (this.getbutton) {
...
//    console.log("SecurenotesDialog#calculateButtonStates " + formelement);
    if (this.choosen && !this.new) {
      this.setButtonStates(true, false, false, false, false, false);
    }
    else if (this.new) {
      ...
      if (formelement === "title") {
        if (value != this.displayedtitle && value != "") {
          ...
          this.setButtonStates(false, true, false, false, true, true);
        } else {
          if (!this.noteChanged) {
            this.setButtonStates(false, false, false, true, true, true);
          }
  ...
  private setButtonStates(getState: boolean, putState: boolean, postState: boolean, deleteState: boolean, dbButtonState: boolean, encryptState: boolean): void {
    this.showGET = getState;
    this.showPUT = putState;
    this.showPOST = postState;
    this.showDELETE = deleteState;
    this.showDbButton = dbButtonState;
  ...
  toggleEncryption() {
    ...
    if (this.raw) {
      ...
      // decrypt
      this.displayedNote = this.cyptoService.decrypt(selectedNote.note);
  ...
  get() {
    ...
    this.secureNotesService.get(choice).subscribe(secureNote => {
      const text = this.cyptoService.decrypt(secureNote.note);
