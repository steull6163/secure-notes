...
export class CryptoService {
  ...
  // for AES decryption of private key should come from user input
  private encryptedPassword: string = "1234567890";
  // should come from server (private key should be AES encrypted)
...
//        this.setKeys(keys);
  ...
  private setKeys(keys: Keys): void {
    ...
    // de-base64 and decrypt the private key
    const decodedPK = CryptoJS.enc.Base64.parse(keys.privateKey).toString(CryptoJS.enc.Utf8);
    this.privateKey = CryptoJS.AES.decrypt(decodedPK, this.encryptedPassword).toString(CryptoJS.enc.Utf8);
